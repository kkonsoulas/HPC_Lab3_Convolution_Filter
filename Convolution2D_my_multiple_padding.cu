...
__global__ void convolutionRowGPU(float *d_Dst, float *d_Src, float *d_Filter, 
  ...
  int y = threadIdx.y + blockDim.y * blockIdx.y + filterR;
      ...
      for (k = -filterR; k <= filterR; k++) {
        ...
        // if (d >= 0 && d < imageW) {
        sum += d_Src[y * (imageW + filterR) + d] * d_Filter[filterR - k];
        // }     

        d_Dst[y * (imageW+ filterR) + x] = sum;
...
__global__ void convolutionColumnGPU(float *d_Dst, float *d_Src, float *d_Filter,
  ...
  int k;
      ...
      for (k = -filterR; k <= filterR; k++) {
        ...
        // if (d >= 0 && d < imageH) {
        sum += d_Src[d * (imageW + filterR) + x] * d_Filter[filterR - k];
        // }   

        //d_Src[d* (ImageW+2*filterR) + x] 
        d_Dst[(y-filterR)*imageW + (x-filterR)] = sum;
...
int main(int argc, char **argv) {
  ...
  *d_OutputGPU;
    ...
    cudaMalloc((void**) &d_Buffer,(imageW + filter_radius) * (imageH +2*filter_radius) *sizeof(float));
    cudaMalloc((void**) &d_Input,(imageW + filter_radius) * (imageH +2*filter_radius)*sizeof(float));
    ...
    float * h_Input2     = (float *)calloc((imageW + filter_radius) * (imageH + 2*filter_radius) , sizeof(float));
    for(int i = 0 ; i < imageH ; i++){
      memcpy(
        &h_Input2[i*(imageW + filter_radius) +filter_radius*(imageW + filter_radius)  + filter_radius],
        &h_Input[i*imageW] , imageW*sizeof(float));
    }

    //code to check if the array is initialised correctly 
    // for(int i=0 ;i<imageW ;i++){
    //   for(int j=0 ; j <imageW ; j++){
    //     printf("%6.2lf ",h_Input[i*imageW + j]);
    //   }
    //   printf("\n");
    // }
    // printf("\n");
    // for(int i=0 ;i<imageW + 2*filter_radius ;i++){
    //   for(int j=0 ; j <imageW + filter_radius ; j++){
    //     printf("%6.2lf ",h_Input2[i*(imageW+filter_radius) + j]);
    //   }
    //   printf("\n");
    // }
    ...
    cudaMemset(d_Buffer,0,(imageW + filter_radius) * (imageH +2*filter_radius) *sizeof(float));
    cudaEventRecord(start);
    cudaMemcpy(d_Filter, h_Filter, FILTER_LENGTH * sizeof(float), cudaMemcpyHostToDevice);


    cudaMemcpy(d_Input, h_Input2,(imageW + filter_radius) * (imageH +2*filter_radius)*sizeof(float), cudaMemcpyHostToDevice);
